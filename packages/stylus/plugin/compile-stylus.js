...
function resolvePath(path) {
  ...
  if (paths.length === 0) {
    console.log(`stylus glob could not find paths in ${paths}, trying **/${path}`)
    console.time('stylus glob.sync')
    paths = glob.sync(`**/${path}`);
    console.timeEnd('stylus glob.sync')
    console.log(`stylus glob finished searching **/${path}`)
